...
“All units: Vannelet is on the run. They are driving a blue car with a numberPlate of AYPCAL.  Pursue at speed.
...
let roadWidth;
let roadLeftEdge;
let Lane_Coordinate_A;
let Lane_Coordinate_B;
let carImages = {};
let suspect;

let TrafficObjects_Data = [
  {
    x: 300,
    y: 0,
    distTravelled: -200,
...
function preload() {
  let carTypes = ["detective", "redCar", "greenCar", "blueCar", "whiteCar"];
...
function draw() {
  ...
  if (!chaseCarObject.tailingSuspect && !chaseCarObject.isArrestingSuspect) {
    move_vehicle();
    for (let i = 0; i < TrafficObjects_Data.length; i++) {
      let b2b = searchVehicle_isAhead(chaseCarObject, TrafficObjects_Data[i]);
  ...
  if (suspect) {
    if (!suspect.isArrested) {
      suspect.accelAmt = 5;
      for (let i = 0; i < TrafficObjects_Data.length; i++) {
        let b2b = searchVehicle_isAhead(suspect, TrafficObjects_Data[i]);
        if (b2b) {
...
function turn_motor(car) {
  ...
  for (let i = car.exhaust.length - 1; i >= 0; i--) {
...
function drawExhaust(car) {
  p5.noStroke();
  for (let i = 0; i < car.exhaust.length; i++) {
    let alpha = map(car.exhaust[i].size, 0, 40, 50, 0);
    p5.fill(125, alpha);
    p5.ellipse(car.exhaust[i].x + 20, car.exhaust[i].y, car.exhaust[i].size);
  }
}
