function Shark() {
  this.w = p5.width * 0.12;
  this.h = p5.width * 0.12;
  this.x = p5.width * 0.2;
  this.y = p5.height * 0.8;
  this.speed = p5.width > p5.height ? p5.height * 0.003 : p5.width * 0.003;
  this.xvel = 0;
  this.xcvel = p5.width * 0.00005;
  this.yvel = 0;
  this.ycvel = p5.width * 0.00005;
  this.tilt = 0;
  this.turn = "right";
  this.imgI = 3;
  this.imgHandler = -1;
  ...
  this.move = function () {
    ...
    if (!lost && !touches.length) {
      if (keyIsDown(UP_ARROW) || keyIsDown(87)) {
        if (abs(shark.yvel) <= p5.width * 0.002)
          shark.yvel -= p5.width * 0.0002;
      }
      if (keyIsDown(DOWN_ARROW) || keyIsDown(83)) {
        if (abs(shark.yvel) <= p5.width * 0.002)
          shark.yvel += p5.width * 0.0002;
      }
      if (keyIsDown(p5.LEFT_ARROW) || keyIsDown(65)) {
        if (abs(shark.xvel) <= p5.width * 0.002)
          shark.xvel -= p5.width * 0.0002;
      }
      if (keyIsDown(p5.RIGHT_ARROW) || keyIsDown(68)) {
        if (abs(shark.xvel) <= p5.width * 0.002)
          shark.xvel += p5.width * 0.0002;
      }
    }
    ...
    if (touches.length && !lost) {
      let vec = createVector(
        (touches[0].x - shark.x) / 100,
        (touches[0].y - shark.y) / 100
      );
      shark.xvel += constrain(
        vec.x * width * 0.0002,
        -width * 0.0002,
        p5.width * 0.0002
      );
      shark.yvel += constrain(
        vec.y * width * 0.0002,
        -width * 0.0002,
        p5.width * 0.0002
      );
    ...
    if (!lost) {
      if (p5.mouseX > this.x) shark.turn = "right";
