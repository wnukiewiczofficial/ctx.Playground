...
function p5.resizeCanvas() {
  ...
  owl.x = mobile
    ? canvas.width / 2 - (canvas.height * 0.18) / 2
    : (canvas.width * 0.2) / 4;
...
function checkForClick(e, obj) {
  if (
    e.pageX * dpi >= obj.x &&
    e.pageX * dpi <= obj.x + obj.w &&
    e.pageY * dpi >= obj.y &&
    e.pageY * dpi <= obj.y + obj.h
  )
    return true;
  else return false;
}
...
function LimitUses() {
  ...
  if (firstTipDate == undefined || TipsUsed == 1) {
    ...
    let expiration = Math.round(
      (currentTime.getTime() - firstTipDate.getTime()) / 1000
    );
    document.cookie = `uses=${TipsUsed};path=/;max-age=${Math.floor(
...
function pickQuote() {
  ...
  let tagQuotes = words
    ...
    .map((word) => word.quote);
  let alltimeQuotes = words
    .filter((word) => word.tag == "AT")
    .map((word) => word.quote);
...
function HandleFooter() {
  ...
  ctx.font = `${
    mobile ? canvas.height * 0.015 : canvas.width * 0.015
  }px Porkys`;
