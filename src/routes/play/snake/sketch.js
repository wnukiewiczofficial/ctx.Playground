...
var snake = []; // Snake body
var apples = [];

var fieldSize; // Size of the field where an apple or snake fits in
var direction = { x: 0, y: 0 };

function setup(p5, canvasParentRef) {
  /// Canvas is always a rectangle
  p5.createCanvas(window.innerWidth, window.innerHeight);
  if (p5.width > p5.height) p5.resizeCanvas(height / 2, p5.height / 2);
  else if (p5.height > p5.width) p5.resizeCanvas(p5.width / 2, p5.width / 2);

  fieldSize = p5.width / 20;
  ellipseMode(CORNER);

  // Snake head on a p5.random position
  snake.push({ x: p5.floor(p5.random(0, 20)), y: p5.floor(p5.random(0, 20)) });
...
function UpdateSnake() {
  ...
  if (SnakeLoses()) {
    //Restart the game
    snake = [{ x: p5.floor(p5.random(0, 20)), y: p5.floor(p5.random(0, 20)) }];
    direction = { x: 0, y: 0 };
    apples = [];
    for (let i = 0; i < 2; i++) {
      apples.push({
